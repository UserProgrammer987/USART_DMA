...
uint8_t Rx_Modbus_RS485[RECEIVER_SIZE];
uint8_t c_Rx_Modbus_RS485 = 0;

uint8_t Tx_Modbus_RS485[TRANSCEIVER_SIZE];
uint8_t c_Tx_Modbus_RS485;
...
void Modbus_Uart_DMA_Init(void) {
	// Enable DMA for UART
	USART1->CR3 |= USART_CR3_DMAR | USART_CR3_DMAT;
	// Enable Idle interrupt to catch end of incoming transmission
	USART1->CR1 |= USART_CR1_IDLEIE;
	
	//Config RX DMA
	DMA1_Channel2 -> CCR &= ~DMA_CCR_EN;
	DMA1_Channel2 -> CNDTR = RECEIVER_SIZE;
	DMA1_Channel2 -> CCR |= DMA_CCR_MINC; 
	DMA1_Channel2 -> CMAR = (uint32_t)Rx_Modbus_RS485;
...
{	
  ...
  for(;;)
  {
		if(Ready_To_ModBus)
		{
			ModBusRTU_PR(Rx_Modbus_RS485, c_Rx_Modbus_RS485, Tx_Modbus_RS485, RS485_U0_send);
			Ready_To_ModBus = 0;
		}
		HAL_Delay(1);
  }
